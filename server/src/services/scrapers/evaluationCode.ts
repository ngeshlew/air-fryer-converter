// Pure JavaScript evaluation code as strings to avoid TypeScript compilation issues
// These are used in page.evaluate() to avoid __name errors

export function getRecipeEvaluationCode(baseUrl: string): string {
  return `
    (function() {
      ...
      const title = getText('h1') || getText('[class*="recipe-header"] h1') || getText('[class*="title"]') || getText('[class*="Title"]') || getText('[class*="RecipeTitle"]');
      ...
      if (ingredients.length === 0) {
        ...
        if (ingredients.length === 0) {
          ingredients = getAllText('[data-test*="ingredient"] li');
        }
        if (ingredients.length === 0) {
          ingredients = getAllText('ul li').filter(function(text) {
            return text.match(/\\d+/) && (text.includes('g') || text.includes('ml') || text.includes('tbsp') || text.includes('tsp') || text.includes('Banana') || text.includes('kg'));
          });
        }
      }
      ...
      if (instructions.length === 0) {
        ...
        if (instructions.length === 0) {
          instructions = getAllText('[data-test*="method"] li');
        }
        if (instructions.length === 0) {
